...
const Newsletter: React.FC<Props> = ({ title, pagePrivacy, mailchimpSignupUrl, images }) => {
  const { loading, error, success, message, handleSubmit } = useMailChimpForm(mailchimpSignupUrl)
  ...
  const inputClass = 'w-full p-2 border border-gray-300 rounded-md text-[16px] lg:text-xs'
  return (
        ...
        <div className="w-full">
          ...
          {success ? (
            ...
            >
              ...
              <label
                htmlFor="agree"
                className={twMerge(labelClass, 'not-sr-only my-2 flex items-center text-xs')}
              >
                <input
                  type="checkbox"
                  name="agree"
              ...
              {message && <div className="break-words font-medium">{message}</div>}
              <button type="submit" className="bg-key-500 hover:bg-key-600 text-white">
